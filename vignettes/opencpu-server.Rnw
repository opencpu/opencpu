...
\subsection{Separating analysis from web development}
...
\end{verbatim}
This will automatically enable/disable OpenCPU in Apache2 and restart the server. If the \texttt{opencpu-cache} package has been installed this server can be controlled separately:
\begin{verbatim}
...
\noindent After restarting the OpenCPU service, the packages are accessible through the API:\\
...
\end{verbatim}
The file contains comments with further instructions. On the other hand, on a server which does have \texttt{opencpu-cache} installed, the certificates need to be configured in the following \texttt{Nginx} configuration file: 
...
\noindent Apart from R objects and manuals, OpenCPU also hosts any static files in package: \\
...
\noindent In a very similar fashion we can produce a plot:
...
\end{verbatim}
In this case, a successful call will return 200 (instead of 201), and the response body contains the output from the function in JSON; no need to do an additional \texttt{GET} request. However in most cases, the two-step procedure is preferred.\\
