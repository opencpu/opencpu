...
%\VignetteIndexEntry{OpenCPU Server Manual}
...
Yet another possibility is to install Ubuntu Server on a virtual machine within another OS. For example, the free VMware Player or Oracle Virtualbox are available for Windows, and on OSX we can use Virtualbox or Parallels to run virtual machines. This way Ubuntu and \OpenCPU can safely be installed on top of a Windows or Mac system. 

To run the \OpenCPU cloud server on Redhat Enterprise Linux or CentOS we recommend KVM. Isolating the \OpenCPU server within a virtual machine is nice from a security point of view and prevents \OpenCPU or \R from interfering with other software on your server. Furthermore KVM provides nice control over hardware resources for guest machines. The \OpenCPU public demo servers run as virtual machines on a KVM hypervisor which has resulted in excellent performance, smooth upgrades and no major incidents for several years.
